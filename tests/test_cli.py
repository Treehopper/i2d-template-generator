...
def test_data_dir_defaults_to_slash_data_absent_env_var(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    monkeypatch.delenv(cli.DATA_DIR_ENV_VAR, raising=False)
    args = cli.build_parser().parse_args([])
    assert args.data_dir == cli.DEFAULT_DATA_DIR == Path("/data")


def test_data_dir_env_var_overrides_the_default(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    monkeypatch.setenv(cli.DATA_DIR_ENV_VAR, str(tmp_path))
    args = cli.build_parser().parse_args([])
    assert args.data_dir == tmp_path


def test_data_dir_flag_beats_the_env_var(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setenv(cli.DATA_DIR_ENV_VAR, str(tmp_path / "from-env"))
    override = tmp_path / "from-flag"
    args = cli.build_parser().parse_args(["--data-dir", str(override)])
    assert args.data_dir == override


def test_omitted_pdfs_default_to_every_pdf_in_data_dir(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    (tmp_path / "a.pdf").write_bytes(b"fake")
    (tmp_path / "b.pdf").write_bytes(b"fake")
    (tmp_path / "not-a-pdf.txt").write_bytes(b"fake")
    monkeypatch.setattr(cli.extract_mod, "extract_text", lambda path, backend: PERSONAL_TEXT)
    ...
    assert (tmp_path / "a.anon.txt").exists()
    assert (tmp_path / "b.anon.txt").exists()


def test_store_and_out_default_under_data_dir(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    (tmp_path / "a.pdf").write_bytes(b"fake")
    text = "Fake Corp\nGesamtbetrag: 1,00 EUR\n"
    monkeypatch.setattr(cli.extract_mod, "extract_text", lambda path, backend: text)
    provider = RecordingProvider({"issuer": "Fake Corp", "keywords": ["Fake Corp"], "fields": {}})
    ...
    assert (tmp_path / "pseudonyms.yml").exists()
    assert (tmp_path / "draft-template.yml").exists()


def test_no_pdfs_found_in_data_dir_raises_a_clear_error(tmp_path: Path) -> None:
    with pytest.raises(SystemExit, match="no PDF files found"):
        cli.run(data_dir=tmp_path, backend="text", no_ai=True, use_defaults=True)


def test_explicit_pdfs_are_not_overridden_by_the_data_dir_glob(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    ...
    assert chosen.with_suffix(".anon.txt").exists()
    assert not (tmp_path / "other.anon.txt").exists()
